package ru.qeekbrains.courses.java.HomeWork_8;
import javax.swing.*;
import java.awt.*;
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;

public class TicTacToeWisualPlus extends JComponent {
    ...
    public static void main(String[] args) {
        ...
        TicTacToeWisualPlus game = new TicTacToeWisualPlus();
        window.add(game);
    }
    ...
    protected void processMouseEvent(MouseEvent mouseEvent){
        ...
        if (mouseEvent.getButton()==MouseEvent.BUTTON1){
            ...
            if (field[i][j] == FIELD_EMPTY){
                ...
                if (res!=0){
                    if (res == FIELD_0*3){
                        JOptionPane.showMessageDialog(this, "Победа за кругликами!","Победа", JOptionPane.INFORMATION_MESSAGE);
                    } else if(res == FIELD_X*3){
                        JOptionPane.showMessageDialog(this, "Победа за крестиками!","Победа", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this, "Ничья!","Победа", JOptionPane.INFORMATION_MESSAGE);
                    }
                    initGame();
                    repaint();
                }
    ...
    void drawX(int i, int j, Graphics graphics){
        ...
        int x = i *dw;
        int y = j *dh;
        graphics.drawLine(x,y,x+dh,y+dh);
   ...
   int checkState() {
        ...
        if (diag == FIELD_0*3 || diag == FIELD_0*3){return diag;}
        if (diag2 == FIELD_0*3 || diag2 == FIELD_0*3){return diag2;}
       int check_i, check_j;
