...
import ru.qeekbrains.courses.java_2021_2.HW_7.constants.Constants;
...
public class ClientHandler {
        ...
        while (true) {
            ...
            if (str.startsWith(Constants.AUTH_COMMAND)) {
                ...
                if (nick != null) {
                    ...
                    //Авторизовались
                    if(server.authorizationCheck(nick)){
                        name = nick;
    ...
    private void readMessage() throws IOException {
        while (true) {
            ...
            if (messageFromClient.startsWith(Constants.PERSONAL_MESSGE)) {
                List<String> tokens = Arrays.asList(messageFromClient.split("\\s+"));
                String name = tokens.get(1);
                messageFromClient = "";
                for (int i=2; i<tokens.size();i++){
                    messageFromClient = messageFromClient + tokens.get(i) + " ";
                }
                server.personalMessage(name, name + ": " + messageFromClient);
            } else {
